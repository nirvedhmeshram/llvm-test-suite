...
half B[MATRIX_K][MATRIX_N];
float C[MATRIX_M][MATRIX_N];
float D[MATRIX_M][MATRIX_N];

void matrix_multiply_ref(half *A_mem, half *B_mem, float *C_mem, int M, int N,
                         int K) {
  // tiling
  for (int m = 0; m < M; m++)
    for (int n = 0; n < N; n++) {
...
int main() {
  ...
  big_matrix<half, MATRIX_K, MATRIX_N> MB((half *)&B);
  matrix_multiply(MC, MA, MB);
  matrix_multiply_ref((half *)A, (half *)B, (float *)D, MATRIX_M, MATRIX_N,
                      MATRIX_K);
